...
| 文档 | 适合什么时候看 | 内容 |
| --- | --- | --- |
| [小卖铺产品介绍.html](docs/小卖铺产品介绍.html) | 初次了解、向其他卖家介绍 | 产品定位、五大亮点、高清演示截图、订单流程与配置概览 |
| [小卖铺使用与配置手册.html](docs/小卖铺使用与配置手册.html) | 准备安装、日常操作、遇到问题 | 18 个章节：API 获取、两种部署方式、逐页面操作、供货商协作、备份迁移与故障排查 |
...
两份文件均内嵌样式和高清演示截图，可离线阅读和转发，不需要先启动小卖铺。将两个 HTML 放在同一个文件夹，宣传页底部的“详细使用手册”就能直接跳转。手册支持目录导航、命令复制及浏览器打印 / 另存为 PDF；外部下载和反馈链接需要联网。
...
![待发货：商品图片、规格成色与供货商分组导出](docs/images/shipping-current.png)

<details>
<summary>查看文字发货清单：复制发送或下载 TXT</summary>

![文字发货清单与专属上传入口](docs/images/shipping-list-current.png)
...
![利润统计与逐单计算](docs/images/profits-current.png)
