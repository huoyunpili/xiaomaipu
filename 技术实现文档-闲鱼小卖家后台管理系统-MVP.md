...
- 主导航：工作台、订单、渠道销售、客户、商品、库存与采购、供应商、资金与利润、数据导入、系统设置。
...
- 密码使用 Django 支持的强哈希器；管理员初始密码必须首次登录修改。
...
初始化生成随机 Django 密钥、数据库口令及独立管理员；无通用正式密码。多次初始化不覆盖配置/资料。启动流程：依赖及端口/目录检查→实例锁→数据库就绪→检查迁移版本→启动 Web/Worker/Beat→健康检查→打开浏览器。失败显示失败服务及安全日志，不将未就绪状态显示为已启动。
