    ...
    def test_login_edit_audit_and_mobile(self):
        ...
        with sync_playwright() as playwright:
            ...
            for label in ("工作台", "待发货", "待完成", "退款售后", "利润统计", "全部订单", "设置"):
                expect(
                    page.get_by_role("navigation", name="主导航").get_by_text(label, exact=True)
                ).to_be_visible()
            ...
            page.get_by_role("link", name="本地店铺与备份", exact=False).click()
            ...
            page.get_by_text("设置", exact=True).click()
            page.get_by_role("link", name="操作记录", exact=False).click()
